...
public class MoaApiService : IMoaApiService
{
        ...
        string? cropName, string? market, int top, int skip)
    {
        ...
        {
            _logger.LogError(ex, "HTTP error fetching MOA data from {Endpoint}", FarmTransEndpoint);
            throw;
        }
        catch (JsonException ex)
        {
            _logger.LogError(ex, "JSON deserialization error while parsing MOA response");
            throw;
        }
        finally
        {
    ...
    private static string ToRocDate(DateTime date)
    {
        ...
        if (date.Year < 1912)
        {
            throw new ArgumentException($"Date {date:yyyy-MM-dd} is before Republic of China era (1912)", nameof(date));
        }
