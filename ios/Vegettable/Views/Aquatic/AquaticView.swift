...
struct AquaticView: View {
    @State private var prices: [AquaticPrice] = []
    ...
    private var filtered: [AquaticPrice] {
        prices.filter { item in
            let matchesMarket = selectedMarket == "全部" || item.marketName == selectedMarket
            let matchesSearch = searchText.isEmpty || item.fishName.contains(searchText)
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    if isLoading && prices.isEmpty {
                        ...
                        Spacer()
                        Text(searchText.isEmpty ? "目前無漁產品行情資料" : "找不到相關漁產品")
                            .foregroundColor(AppColors.textTertiary)
                        Spacer()
                    } else {
                        List {
                            ForEach(filtered) { item in
                                AquaticRow(item: item)
            ...
            .navigationTitle("漁產品行情")
            .navigationBarTitleDisplayMode(.large)
            .searchable(text: $searchText, prompt: "搜尋魚貨名稱")
    ...
    private func loadPrices() async {
        ...
        errorMessage = nil
        do {
            let result = try await ApiClient.shared.fetchFishPrices()
            await MainActor.run {
                prices = result
                isLoading = false
            }
...
struct AquaticRow: View {
    let item: AquaticPrice
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                Text("元/公斤")
                    .font(.caption2)
                    .foregroundColor(AppColors.textTertiary)
                Text("量: \(String(format: "%.0f", NSDecimalNumber(decimal: item.volume).doubleValue)) 公斤")
                    .font(.caption2)
                    .foregroundColor(AppColors.textTertiary)
            }
        }
